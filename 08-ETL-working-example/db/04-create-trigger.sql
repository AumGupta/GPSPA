...
*/
CREATE Or REPLACE FUNCTION sa.insert_ride_in_pa()
RETURNS TRIGGER AS
$$
BEGIN
	...
	VALUES (now(), new.pickup_datetime, new.dropoff_datetime, new.passenger_count, 
			...
			ST_SetSRID(ST_MakePoint(new.pickup_longitude, new.pickup_latitude), 4326),
			ST_SetSRID(ST_MakePoint(new.dropoff_longitude, new.dropoff_latitude), 4326));
	RETURN new;
END;
$$
LANGUAGE 'plpgsql';
